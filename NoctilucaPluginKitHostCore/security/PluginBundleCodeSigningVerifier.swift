...
/// 코드 서명 검증 결과
public enum CodeSigningVerificationResult: Sendable {
...
public struct PluginBundleCodeSigningVerifier {
    public static let teamUnstablersTeamID = "XHA76UVA95"

    /// 번들의 코드 서명을 검증한다.
    public static func verify(bundleURL: URL) -> CodeSigningVerificationResult {
    ...
    ) -> Result<Void, PluginBundleCodeSigningError> {
        switch policy {
        case .disallowAll:
