...
public struct WindowHints: OptionSet, Hashable, Equatable {
    ...
    public static let systemUI = Self(rawValue: 1 << 0)
    
    
    /// 이 윈도우는 시스템에 의해 응답하지 않는 것으로 보고되었습니다.
    public static let notResponding = Self(rawValue: 1 << 1)
    
    /// 이 윈도우는 시스템 정책에 의해 조작이 불가능할 것으로 예상됩니다. (다른 유저 세션의 윈도우, 고권한 프로세스의 윈도우, UAC 등)
    public static let inaccessible = Self(rawValue: 1 << 2)
    ...
    public static let protectedContent = Self(rawValue: 1 << 3)
    ...
    public static let aggressiveProtectedContent = Self(rawValue: 1 << 4)
    
    /// 이 윈도우는 현재 다른 창에 가려져 있거나 해서 화면에 보이지 않는 상태입니다.
    public static let notVisibleOnScreen = Self(rawValue: 1 << 5)
    
    /// 이 윈도우는 자주 변경되지 않는 정적 콘텐츠를 포함하고 있습니다.
    public static let staticContent = Self(rawValue: 1 << 8)
    
    /// 이 윈도우는 빠르게 갱신되는 동적 컨텐츠 (예: 비디오 플레이어, 게임 등)를 포함하고 있습니다.
    public static let dynamicContent = Self(rawValue: 1 << 9)
    
    /// 이 윈도우는 그림자를 포함하고 있습니다.
    /// - 윈도우 외곽에 그림자가 있는 경우에 유용합니다.
    /// - AppStream (가칭) 같은 기능을 구현할 때, 클라이언트 측에서 대신 shadow를 그리도록 할 수 있습니다.
    public static let hasShadow = Self(rawValue: 1 << 12)
    
    /// 이 윈도우는 투명한 영역을 포함하고 있습니다.
    /// - border-radius가 설정되었거나, 부분적으로 투명한 UI를 포함하는 윈도우에 유용합니다.
    public static let hasTransparency = Self(rawValue: 1 << 13)
