    ...
    var summaryTab: some View {
        VStack(alignment: .leading) {
            ...
            case .fingerprintMismatch(let expectedFingerprint, let actualFingerprint):
                VStack(alignment: .leading, spacing: 6) {
                    Text(markdown: String(localized: "auth.identity.danger", defaultValue: "위험"))
                        .font(.title)
                        .foregroundStyle(.red)
                    Text(markdown: String(localized: "auth.identity.fingerprint_mismatch.title", defaultValue: "서버 인증서 지문이 일치하지 않습니다!"))
                        .font(.title2)
                }
                ...
                VStack(alignment: .center, spacing: 16) {
                    ...
                    VStack(alignment: .leading) {
                        Text(markdown: String(localized: "auth.identity.expected_fingerprint", defaultValue: "이 클라이언트가 기억하고 있는 인증서 지문"))
                            .font(.system(size: 14))
                            .bold()
                    ...
                    VStack(alignment: .leading) {
                        Text(markdown: String(localized: "auth.identity.actual_fingerprint", defaultValue: "서버에서 제시한 인증서 지문"))
                            .font(.system(size: 14))
                            .bold()
            ...
            case .none:
                Spacer()
                VStack(alignment: .leading, spacing: 6) {
                    Text(markdown: String(localized: "auth.identity.warning", defaultValue: "경고"))
                        .font(.title)
                        .foregroundStyle(.yellow)
                    Text(markdown: String(localized: "auth.identity.untrusted.title", defaultValue: "서버의 인증서를 검증할 수 없습니다."))
                        .font(.title2)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
                .bold()
        ...
        ScrollView {
            VStack {
                ...
                case .fingerprintMismatch(_, _):
                    ...
                    DisclosureGroup {
                        VStack(alignment: .leading, spacing: 4) {
                            Text(markdown: String(localized: "auth.identity.help.why_warning.mismatch.body_1", defaultValue: "이 경고는 다음과 같은 상황에서 발생할 수 있습니다:"))
                            Text(markdown: String(localized: "auth.identity.help.why_warning.mismatch.body_2", defaultValue: "• 서버 관리자가 인증서를 교체하였습니다."))
                            Text(markdown: String(localized: "auth.identity.help.why_warning.mismatch.body_3", defaultValue: "• **중간자 공격**(MITM)의 가능성이 있습니다: 누군가가 악의적인 목적으로 통신 내용을 훔쳐보거나 변조하려 할 수도 있습니다."))
                        }
                        .padding(.vertical, 8)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .font(.callout)
                    ...
                    DisclosureGroup {
                        VStack(alignment: .leading, spacing: 4) {
                            Text(markdown: String(localized: "auth.identity.help.consequences.mismatch.body_1", defaultValue: "최악의 경우, 비밀번호나 기밀 정보가 유출될 수 있습니다."))
                        }
                        .padding(.vertical, 8)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .font(.callout)
                    ...
                    DisclosureGroup {
                        VStack(alignment: .leading, spacing: 4) {
                            Text(markdown: String(localized: "auth.identity.help.what_to_do.mismatch.body_1", defaultValue: "• 안전하지 않은 인터넷 환경 (공개된 Wi-Fi 등)에서 작업하고 있다면, 접속을 해제하고 안전이 보장되는 환경에서 다시 시도하십시오."))
                            Text(markdown: String(localized: "auth.identity.help.what_to_do.mismatch.body_2", defaultValue: "• 만약 최근에 인증서를 직접 교체한 기억이 있으시다면 계속 진행하셔도 괜찮습니다."))
                        }
                        .padding(.vertical, 8)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .font(.callout)
                    } label: {
                        Text(markdown: String(localized: "auth.identity.help.what_to_do.title", defaultValue: "그럼 저는 어떻게 해야 하나요?"))
                            .multilineTextAlignment(.leading)
                            .disclosureLabelStyle()
                    }
                    .padding(.bottom, 12)
                ...
                case .none:
                    DisclosureGroup {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text(markdown: String(localized: "auth.identity.help.simple_explanation.untrusted.body_2", defaultValue: "컴퓨터끼리 암호화된 통신을 할 때는, 신분증 (인증서)이 필요합니다. 인증서를 발급 받는 방법은 크게 2가지가 있습니다.\n"))
                            Text(markdown: String(localized: "auth.identity.help.simple_explanation.untrusted.body_3", defaultValue: "• **신뢰받는 인증 기관에서 발급받기**: 지금 보고 계신 것과 같은 경고가 표시되지 않게 되지만, 비용을 지불해야 합니다."))
                            Text(markdown: String(localized: "auth.identity.help.simple_explanation.untrusted.body_4", defaultValue: "• **자가 서명 인증서 발급하기**: 자기 자신의 인증서를 직접 만드는 방법입니다. 다른 컴퓨터들이 진위를 검증할 수 없기 때문에 이러한 오류가 표시됩니다.\n"))
                            Text(markdown: String(localized: "auth.identity.help.simple_explanation.untrusted.body_5", defaultValue: "이번에 접속을 시도한 컴퓨터는 진위를 알 수 없는 신분증을 제시하였습니다."))
                        }
                        .padding(.vertical, 8)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .font(.callout)
                    ...
                    DisclosureGroup {
                        VStack(alignment: .leading, spacing: 4) {
                            Text(markdown: String(localized: "auth.identity.help.why_warning.untrusted.body_1", defaultValue: "이 경고는 다음과 같은 상황에서 표시될 수 있습니다:"))
                            Text(markdown: String(localized: "auth.identity.help.why_warning.untrusted.body_2", defaultValue: "• 자가 서명 인증서를 사용한 경우"))
                            Text(markdown: String(localized: "auth.identity.help.why_warning.untrusted.body_3", defaultValue: "• 시스템의 트러스트 스토어가 낡은 상태인 경우"))
                            Text(markdown: String(localized: "auth.identity.help.why_warning.untrusted.body_4", defaultValue: "• 시스템의 시계가 틀어져 실제 시각과 커다란 차이가 나는 경우"))
                        }
                        .padding(.vertical, 8)
                        .frame(maxWidth: .infinity, alignment: .leading)
                        .font(.callout)
            ...
            TabView(selection: $selectedTab) {
                self.summaryTab
                    .tabItem {
                        ...
                        Text(markdown: String(localized: "auth.identity.tab.warning", defaultValue: "경고"))
                    }
                    .tag(Tab.summary)
                    .id(Tab.summary)
                ...
                self.certificateInfoTab
                    .tabItem {
                        ...
                        Text(markdown: String(localized: "auth.identity.tab.certificate_info", defaultValue: "인증서 정보"))
                    }
                ...
                self.helpTab
                    .tabItem {
                        ...
                        Text(markdown: String(localized: "auth.identity.tab.help", defaultValue: "도움말"))
                    }
                    .tag(Tab.help)
                    .id(Tab.help)
