            ...
            Section {
                SettingsPicker(selection: tlsValidationPolicy) {
                    SettingsPickerItem(value: AppSettings.TLSValidationPolicy.unsafe) {
                        Text(markdown: String(localized: "session-settings.security.tls.unsafe", defaultValue: "수행하지 않기 **(위험!)**"))
                        Text(markdown: String(localized: "session-settings.security.tls.unsafe_desc", defaultValue: "서버의 SSL/TLS 인증서의 유효성 검사를 수행하지 않습니다.\n제 3자가 통신 내용을 훔쳐보거나 변조하는 중간자 공격 (MITM)에 취약하므로 권장하지 않습니다."))
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
                    }

                    SettingsPickerItem(value: AppSettings.TLSValidationPolicy.default) {
                        Text(markdown: String(localized: "session-settings.security.tls.default", defaultValue: "기본"))
                        Text(markdown: String(localized: "session-settings.security.tls.default_desc", defaultValue: "모든 호스트가 신뢰받은 인증 기관으로부터 SSL/TLS 인증서를 발급받을 수 없다는 상황을 감안합니다.\n자가 서명 인증서를 사용한 호스트의 경우 경고 팝업을 표시합니다."))
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
                    }

                    SettingsPickerItem(value: AppSettings.TLSValidationPolicy.strict) {
                        Text(markdown: String(localized: "session-settings.security.tls.strict", defaultValue: "엄격히"))
                        Text(markdown: String(localized: "session-settings.security.tls.strict_desc", defaultValue: "시스템의 트러스트 스토어를 기준으로 서버의 SSL/TLS 인증서를 엄격하게 검사합니다.\n자가 서명 인증서를 사용 중인 호스트의 경우 연결에 실패할 수 있습니다."))
                            .font(.subheadline)
                            .foregroundStyle(.secondary)
                    }
                } label: {
                    Text(markdown: String(localized: "session-settings.security.tls.policy", defaultValue: "유효성 검사 정책"))
                    Text(markdown: String(localized: "session-settings.security.tls.policy_desc", defaultValue: "서버의 SSL/TLS 인증서의 유효성 검사 수준을 설정합니다."))
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
                }
                ...
                Text(markdown: String(localized: "session-settings.security.transport", defaultValue: "트랜스포트 레이어"))
                Text(markdown: String(localized: "session-settings.security.transport_desc", defaultValue: "Noctiluca에서는 QUIC 프로토콜을 사용하여 통신합니다."))
            }
