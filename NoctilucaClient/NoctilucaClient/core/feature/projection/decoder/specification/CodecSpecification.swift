...
extension CodecSpecification {
    ...
    fileprivate var commonDescription: String {
        ...
        case .kHardwareAccelerationAuto:
            entries.append(String(localized: "codec-spec.hw_accel.auto", defaultValue: "서버가 하드웨어 가속 사용 여부를 자동 결정"))
        case .kHardwareAccelerationFalse:
            entries.append(String(localized: "codec-spec.hw_accel.requested", defaultValue: "서버 하드웨어 가속 사용 요청"))
        default:
            entries.append(String(localized: "codec-spec.hw_accel.custom", defaultValue: "서버 하드웨어 가속 사용 정책 사용자 지정"))
        }
