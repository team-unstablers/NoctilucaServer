...
struct RLECodecSpecificationSheet: View {
            ...
            Form {
                Section {
                    ...
                    Slider(
                        ...
                        in: 1...22,
                        step: 1,
                        minimumValueLabel: Text("1 (빠름)"),
                        maximumValueLabel: Text("22 (느림)")
                    ) {
                        Text("Zstd 압축 레벨")
                        if let compressionLevel = Int(specification.options[.compressionLevel]?.rawValue ?? "1") {
                            if compressionLevel >= 20 {
                                Text("**압축 레벨을 Ultra (\(compressionLevel))으로 설정합니다.** 매우 높은 압축률을 제공하지만, 메모리 사용량이 증가합니다.")
                            } else {
                                Text("압축 레벨을 \(compressionLevel)으로 설정합니다.")
                            }
                        }
                    }
    ...
    var body: some View {
        if specification.fourCC != .zrle {
            Text("ASSERTION FAILED: This sheet is only for RLE codec specification.")
        } else {
            __body
        }
    }
