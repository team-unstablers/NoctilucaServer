    ...
    var body: some View {
        Form {
            Section {
                ...
                Text(String(localized: "settings.security.auth_methods.title", defaultValue: "인증 수단"))
                Text(String(localized: "settings.security.auth_methods.description", defaultValue: "이 컴퓨터에 접속할 때 사용할 인증 수단을 설정합니다. 드래그-드롭으로 우선 순위를 변경할 수 있습니다. [더 알아보기…](http://google.com)"))
            }

            Section {
                IntegerField(value: $settings.quicTransport.listenPort) {
                    Text(String(localized: "settings.security.server_port.title", defaultValue: "서버 포트"))
                    Text(String(localized: "settings.security.server_port.description", defaultValue: "Noctiluca가 수신 대기할 포트를 설정합니다."))
                }
                    ...
                    SettingsEntry(title: String(localized: "settings.security.server_certificate.title", defaultValue: "서버 인증서"), subtitle: String(localized: "settings.security.server_certificate.valid_until", defaultValue: "이 인증서는 \(identityInfo.notAfter.formatted(date: .numeric, time: .omitted))까지 유효합니다.")) {
                        VStack(alignment: .trailing) {
                            ...
                            Button {
                                ...
                                Text(String(localized: "settings.security.server_certificate.view_details", defaultValue: "인증서 세부 정보 보기…"))
                            }
                            /*
                ...
                Toggle(isOn: $settings.quicTransport.tlsUseAutoconf) {
                    Text(String(localized: "settings.security.tls_autoconf.title", defaultValue: "서버 인증서를 자동으로 구성하기"))
                    Text(String(localized: "settings.security.tls_autoconf.description", defaultValue: "자가 서명 인증서를 사용하여 QUIC 통신을 암호화합니다."))
                }
                
                if !settings.quicTransport.tlsUseAutoconf {
                    Toggle(isOn: $settings.quicTransport.tlsStrictValidation) {
                        Text(String(localized: "settings.security.tls_strict_validation.title", defaultValue: "엄격한 유효성 검사 사용하기"))
                        Text(String(localized: "settings.security.tls_strict_validation.description", defaultValue: "시스템의 트러스트 스토어를 기준으로 신뢰할 수 없는 인증서를 사용 시 경고를 표시합니다."))
                    }
                ...
                Text(String(localized: "settings.security.transport_layer.title", defaultValue: "트랜스포트 레이어"))
                Text(String(localized: "settings.security.transport_layer.description", defaultValue: "Noctiluca에서는 QUIC 프로토콜을 사용하여 통신합니다. [더 알아보기…](http://google.com)"))
            }

            Section {
                Toggle(isOn: $settings.transport.disableServerVersionAnnouncement) {
                    Text(String(localized: "settings.security.protocol.hide_server_version.title", defaultValue: "서버 버전을 알리지 않기"))
                    Text(String(localized: "settings.security.protocol.hide_server_version.description", defaultValue: "서버 버전을 클라이언트에게 알리지 않습니다."))
                }

                Toggle(isOn: $settings.transport.disableSupportedFeaturesAnnouncement) {
                    Text(String(localized: "settings.security.protocol.hide_features.title", defaultValue: "사용 가능한 기능 목록을 알리지 않기"))
                    Text(String(localized: "settings.security.protocol.hide_features.description", defaultValue: "서버에서 지원하는 기능 목록을 클라이언트에게 알리지 않습니다. 호환성이 떨어질 수 있습니다."))
                }
                TextField(text: $settings.transport.motd, prompt: Text(String(localized: "settings.security.protocol.motd.placeholder", defaultValue: "[부자가 되는 가장 빠른 지름길은, 많은 돈을 버는 것입니다.]")).italic()) {
                    Text(String(localized: "settings.security.protocol.motd.title", defaultValue: "오늘의 메시지 (MOTD)"))
                    Text(String(localized: "settings.security.protocol.motd.description", defaultValue: "클라이언트가 접속할 때 표시할 오늘의 메시지를 설정합니다."))
                }
                TextField(text: $settings.transport.authChallengeMessage, prompt: Text(String(localized: "settings.security.protocol.auth_challenge_message.placeholder", defaultValue: "[Contoso Inc. - 관계자 외 접근을 금합니다.]")).italic()) {
                    Text(String(localized: "settings.security.protocol.auth_challenge_message.title", defaultValue: "인증 요청 시 표시할 메시지"))
                    Text(String(localized: "settings.security.protocol.auth_challenge_message.description", defaultValue: "클라이언트에게 인증 요청 시 표시할 메시지를 설정합니다."))
                }
            } header: {
                Text(String(localized: "settings.security.protocol.title", defaultValue: "프로토콜"))
                Text(String(localized: "settings.security.protocol.description", defaultValue: "Sirius 프로토콜의 동작 방식을 설정합니다. [더 알아보기…](http://google.com)"))
            }
        }
        ...
        .sheet(isPresented: $showCertificateDetailSheet) {
            if let certificate = certificate {
                ...
                Text(String(localized: "settings.security.server_certificate.load_failed", defaultValue: "인증서 정보를 불러올 수 없습니다."))
            }
        }
