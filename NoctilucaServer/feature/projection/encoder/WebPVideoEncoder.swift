...
final class WebPVideoEncoder: VideoEncoder, @unchecked Sendable {
    private let logger = NoctilucaLogger(category: "WebPVideoEncoder")
    ...
    init() {
        self.workerQueue = DispatchQueue(label: "app.noctiluca.server.projection.encoder.webp.worker", qos: .userInitiated)
        self.callbackQueue = DispatchQueue(label: "app.noctiluca.server.projection.encoder.webp.callback", qos: .userInitiated)
    ...
    func prepare(with configuration: VideoEncoderConfiguration) throws {
        ...
        // WebP: kColorFormatAuto는 YUV420과 동일하게 처리
        ...
        if let parsedLevel = Int32(levelString) {
            self.compressionLevel = max(0, min(100, parsedLevel))
        } else {
...
private enum WebPVideoEncoderError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .configInitFailed:
            return "WebP config initialization failed."
        case .pictureInitFailed:
            return "WebP picture initialization failed."
        case .importFailed:
            return "WebP BGRA import failed."
        case .encodingFailed:
...
private extension WebPVideoEncoder {
    func webpQuality() -> Float {
        return Float(max(0, min(100, compressionLevel)))
    }

    func encodeWebP(_ pixelBuffer: CVPixelBuffer, quality: Float) throws -> Data {
        ...
        guard let baseAddress = CVPixelBufferGetBaseAddress(pixelBuffer) else {
            throw WebPVideoEncoderError.pixelBufferLockFailed
        }
        ...
        guard WebPPictureInit(&picture) != 0 else {
            throw WebPVideoEncoderError.pictureInitFailed
        }
        ...
        // 3. BGRA 데이터 import
        let importResult = WebPPictureImportBGRX(
            &picture,
            baseAddress.assumingMemoryBound(to: UInt8.self),
            Int32(bytesPerRow)
        )
        guard importResult != 0 else {
            throw WebPVideoEncoderError.importFailed
        }
        ...
        // 5. 인코딩 실행
        let encodeResult = WebPEncode(&config, &picture)
        guard encodeResult != 0 else {
            WebPMemoryWriterClear(&writer)
            throw WebPVideoEncoderError.encodingFailed
        }
