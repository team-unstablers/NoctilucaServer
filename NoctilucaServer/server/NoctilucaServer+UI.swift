...
extension NoctilucaServer {
    ...
    func handleError(identityLoadError error: QUICServerIdentityLoadError) async {
        ...
        case .conflictingIdentitiesFound:
            alert.title = "중복된 이름의 서버 인증서 발견됨"
            // 꼭 소스가 Keychain일거란 보장은 없다. 하지만 지금은 Keychain에서만 로드한다.
            alert.message = "Keychain에서 '\(identityName)' 이름을 가진 인증서가 여러 개 발견되어 서버를 기동할 수 없습니다.\nKeychain Access 앱으로부터 중복된 인증서들을 제거 후 다시 서버를 기동하여 주십시오."
...
"""
            ...
            alert.addButton(title: "Keychain Access 열기", action: openKeychainAccess)
            
            alert.addButton(title: "자가 서명 인증서를 자동 구성하기") {
                if isAutoconfEnabled {
                    // 어떤 상황이냐면: 자가 서명 인증서가 이미 구성되어 있는데 서버 버그인지 뭔지 자가 서명 인증서가 여러 개 있는거임
                    let alert = NOCAlert()
                    alert.title = "자동 구성을 다시 실시하시겠습니까?"
                    alert.message = "자동 구성을 다시 실시하면 기존에 생성된 인증서가 전부 삭제되고 새로운 인증서가 생성됩니다."
                    
                    alert.addButton(title: "예") {
                         performAutoconfInner()
                    }
                    
                    alert.addButton(title: "아니오") {}
            ...
            alert.addButton(title: "닫기") {}
        case .identityNotFound:
            alert.title = "서버 인증서를 찾을 수 없음"
            
            switch identitySource {
            case .keychain:
                alert.message = "Keychain에서 '\(identityName)' 이름을 가진 서버 인증서를 찾을 수 없습니다."
            case .pemFile:
                alert.message = "지정된 경로에서 서버 인증서를 찾을 수 없습니다."
            default:
                alert.message = "서버 인증서를 찾을 수 없습니다."
            }
            
            alert.addButton(title: "자가 서명 인증서를 자동 구성하기", action: performAutoconfInner)
            
            alert.addButton(title: "닫기") {}
        default:
            alert.title = "서버 인증서 로드 실패"
            alert.message = "알 수 없는 이유로 서버 인증서를 로드하지 못했습니다.\n\(error.localizedDescription)"
